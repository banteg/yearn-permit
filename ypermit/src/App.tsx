...
function App() {
  ...
  const supports_atomic_batch = capabilities.isSuccess && capabilities.data[chain_id].atomicBatch.supported;
  ...
  const user_tokens: Token[] | undefined = useMemo(
    () => user_query.data?.filter((value) => value.latest && (value.token_balance > 1n || value.vault_balance > 1n)),
    [user_query.data],
  );

  const migrateable_vaults: Token[] | undefined = useMemo(
    () => user_query.data?.filter((value) => !value.latest && value.vault_balance > 1n),
    [user_query.data],
  );
