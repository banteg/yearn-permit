...
import { type UseReadContractReturnType, useAccount, useReadContracts } from "wagmi";
...
export function LoadingBunny({
  ...
  return (
    <div className="ml-1 -translate-y-[3px] inline-block">
      <Rabbit
        strokeWidth="1.6"
        className={cn("text-violet-500 inline-block relative w-[1.5rem] h-[1.5rem]", animation)}
      />
    </div>
  );
}
...
export function DeadBunny() {
  return (
    <div className="ml-1 inline-block">
      <Tooltip content="rip">
        <Rabbit strokeWidth="1.6" className="inline-block relative w-[1.5rem] h-[1.5rem] -rotate-[140deg]" />
      </Tooltip>
    </div>
  );
}
...
export function SupportedTokens({
  ...
  return (
    <>
      <Text size="5">
        supports {num_tokens?.toString()} tokens, you have {user_tokens?.length} tokens
      </Text>
    </>
  );
}
