...
export function MigrateVaults({
  ...
  return (
    <Flex gap="4" direction="column">
      ...
      />
      <SelectToken tokens={tokens} selected_token={selected_token} on_select={set_selected} busy={busy} />
      {selected_token && migration_selected && (
        <Flex gap="2" className="items-baseline">
          ...
          />
          <Code truncate>
            <ExplorerAddress address={selected_token.vault}>yv{selected_token.symbol}</ExplorerAddress>
            .withdraw()
          </Code>
        </Flex>
      )}
