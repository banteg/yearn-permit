...
export function MakeDeposit({ token, permit, set_permit, busy, set_busy }: MakeDepositProps) {
  ...
  return (
    <Flex gap="2" className="items-baseline">
      ...
      <Code truncate>
        ...
        {from_wei(permit.message.permitted.amount, token.decimals)}, {permit.message.deadline.toString()},{" "}
        {permit.signature})
      </Code>
    </Flex>
  );
